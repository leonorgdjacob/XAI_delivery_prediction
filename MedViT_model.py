...
print(">>> [MedViT DEBUG] Loaded this exact file!")
...
from utils_MedViT_model import merge_pre_bn
...
class LTB(nn.Module):
    ...
    def forward(self, x):
        ...
        out, attn = self.e_mhsa(out, return_attn=True)
...
def MedViT_small(pretrained=False, pretrained_cfg=None, **kwargs):
    ...
    if pretrained and pretrained_cfg:
        checkpoint = torch.load(pretrained_cfg, map_location='cpu')
        model.load_state_dict(checkpoint['model'])
    return model


@register_model
def MedViT_base(pretrained=False, pretrained_cfg=None,num_classes=2, **kwargs):
    model = MedViT(stem_chs=[64, 32, 64], depths=[3, 4, 20, 3], path_dropout=0.2, **kwargs)
    if pretrained and pretrained_cfg:
        checkpoint = torch.load(pretrained_cfg, map_location='cpu')
        
        # Load state dict while ignoring the final layer 'proj_head'
        state_dict = checkpoint['model']
        
        # Remove 'proj_head' from state_dict (since it's size-mismatched)
        state_dict.pop('proj_head.0.weight', None)  # Removing proj_head weights
        state_dict.pop('proj_head.0.bias', None)    # Removing proj_head bias
        
        # Load the state dict without the proj_head weights
        model.load_state_dict(state_dict, strict=False)

        # Adjust the final classification layer (proj_head) to match your number of classes
        in_features = model.proj_head[0].weight.shape[1]  # Get input features of the final layer
        model.proj_head = torch.nn.Sequential(
            torch.nn.Linear(in_features, num_classes)  # Adjust the output size
        )
    return model
